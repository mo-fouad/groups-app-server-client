...
io.on("connection", socket => {
  ...
  socket.emit("groupsData", dbJson.groups);

  socket.on("addNewGroup", addNewGroupData => {
    addNewGroup(socket, addNewGroupData);
...
submitNotifications = (socket, notificationsMeg, notificationTyp) => {
  console.log(notificationsMeg, notificationTyp);
  socket.emit("Notification", notificationsMeg, notificationTyp);
};
...
addNewGroup = (socket, addNewGroupData) => {
  ...
  if (
    ...
    submitNotifications(socket, "Group Already Exits", "red");
    ...
    submitNotifications(socket, "Group has been added", "green");
